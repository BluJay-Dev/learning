...
​
		...
		for _, reservation := range result.Reservations {
			for _, instance := range reservation.Instances {
				get := ("https://" + *instance.PublicIpAddress + ":8083/engine")
				fmt.Printf("%v\n", get)
​
				tr := &http.Transport{
...
​
				req, err := http.NewRequest("GET", os.ExpandEnv(get), nil)
				if err != nil {
					// handle err
				}
				req.SetBasicAuth(os.ExpandEnv("mwadmin:JM%tx2+23bE7")
